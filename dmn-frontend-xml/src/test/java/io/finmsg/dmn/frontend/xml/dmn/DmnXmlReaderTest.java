...
class DmnXmlReaderTest {
	...
	void readsAllImportAttributesAndIgnoresForeignNamespaceCollisions() {
		...
		assertEquals("https://www.omg.org/spec/DMN/20230324/MODEL/", definitions.getImports(0).getImportType());
	}
	...
	void ignoresForeignNamespaceCollisionsInsideModelElements() {
		...
		assertEquals("right", definitions.getDrgElements(0).getInputData().getVariable().getNode().getId());
		assertEquals("Right", definitions.getDrgElements(0).getInputData().getVariable().getNode().getName());
	}
	...
	void returnsStructuredDiagnosticForMalformedXml() {
		...
		DmnReadResult result = new DmnXmlReader().readResult("<definitions>".getBytes(), options);

		assertTrue(result.model().isEmpty());
		assertTrue(result.hasErrors());
		assertEquals("DMN-XML-003", result.diagnostics().get(0).getCode());
		assertEquals(DiagnosticSeverity.DIAGNOSTIC_SEVERITY_FATAL, result.diagnostics().get(0).getSeverity());
		assertEquals("memory:broken.dmn", result.diagnostics().get(0).getLocation().getSystemId());
	}
	...
	void rejectsOversizedStreamsWithoutParsingThem() {
		...
		DmnReadResult result = new DmnXmlReader().readResult(new ByteArrayInputStream(xml), options);

		assertTrue(result.model().isEmpty());
		assertEquals("DMN-XML-001", result.diagnostics().get(0).getCode());
		assertEquals("memory:large.dmn", result.diagnostics().get(0).getLocation().getSystemId());
	}

	@Test
	void legacyReadApiStillThrowsForInvalidInput() {
		assertThrows(RuntimeException.class, () -> new DmnXmlReader().read("<definitions>".getBytes()));
	}

	@Test
	void capturesSourceLocationsOnlyWhenRequested() {
		DmnReadOptions options = new DmnReadOptions(1024, "memory:located.dmn").withSourceLocations(true);
	...
	void reportsUnsupportedDecisionLogicSeparatelyFromMalformedXml() {
		...
		DmnReadResult result = new DmnXmlReader().readResult(xml.getBytes(),
				new DmnReadOptions(4096, "memory:unsupported.dmn"));
	...
	void resolvesQNameTypeReferenceInElementScope() {
		...
		var named = new DmnXmlReader().read(xml.getBytes()).getDrgElements(0).getInputData().getVariable().getType()
				.getNamed();
	...
	void reportsUnknownRootDmnContentAsUnsupported() {
		...
		DmnReadResult result = new DmnXmlReader().readResult(xml.getBytes(),
				new DmnReadOptions(4096, "memory:artifact.dmn"));
	...
	void reportsUnknownNestedDmnContentAsUnsupported() {
		...
		DmnReadResult result = new DmnXmlReader().readResult(xml.getBytes(),
				new DmnReadOptions(4096, "memory:nested.dmn"));
