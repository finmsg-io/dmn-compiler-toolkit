...
public final class NodeReader {
	...
	public Node read(XmlCursor cursor) {
		...
		if (cursor.captureSourceLocations()) {
			...
			if (cursor.line() > 0)
				location.setLine(cursor.line());
			if (cursor.column() > 0)
				location.setColumn(cursor.column());
			if (cursor.offset() >= 0)
				location.setOffset(cursor.offset());
	...
	private void readMetadata(XmlCursor cursor, Node.Builder builder) {
		...
		do {
			switch (cursor.documentLocalName()) {
				case "documentation" -> builder.setDocumentation(Documentation.newBuilder().setText(cursor.text()));
	...
	private ExtensionElements readExtensions(XmlCursor cursor) {
		...
		do {
			ExtensionElement.Builder element = ExtensionElement.newBuilder().setNamespace(cursor.namespaceUri())
					.setName(cursor.localName()).setValue(cursor.text());
			cursor.attributes()
					.forEach(attribute -> element
							.addAttribute(ExtensionAttribute.newBuilder().setNamespace(attribute.namespaceUri())
									.setName(attribute.localName()).setValue(attribute.value())));
