...
final class OutputClauseWriter {
	...
	void write(XmlEmitter xml, OutputClause value) {
		...
		if (value.hasOutputValues()) {
			InputClauseWriter.writeFeel(xml, "outputValues", value.getOutputValues().getText().getText(), "");
		}
		if (value.hasDefaultOutputEntry()) {
			if (!value.getDefaultOutputEntry().hasText() || !value.getDefaultOutputEntry().getText().hasFeel()) {
				throw new XmlWriteException("Default output entry has no writable FEEL source text.");
			}
			InputClauseWriter.writeFeel(xml, "defaultOutputEntry",
