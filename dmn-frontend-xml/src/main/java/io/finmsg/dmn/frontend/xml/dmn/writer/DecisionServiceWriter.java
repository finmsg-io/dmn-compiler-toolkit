...
public final class DecisionServiceWriter implements XmlWriter<DecisionService> {
  ...
  public void write(XmlEmitter xml, DecisionService value) {
    ...
    value.getInputDecisionsList()
        .forEach(reference -> referenceWriter.write(xml, "inputDecision", reference));
    value.getInputDataList()
        .forEach(reference -> referenceWriter.write(xml, "inputData", reference));
    xml.endElement();
  }
}
