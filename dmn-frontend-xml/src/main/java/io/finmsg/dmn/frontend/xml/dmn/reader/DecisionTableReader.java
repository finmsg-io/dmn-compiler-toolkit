...
public final class DecisionTableReader {
		...
		if (cursor.firstChild()) {
			do {
				switch (cursor.documentLocalName()) {
					...
					case "rule" ->
						builder.addRules(decisionRuleReader.read(cursor).toBuilder().setRuleIndex(ruleIndex++).build());
	...
	private void readPreferredOrientation(XmlCursor cursor, DecisionTable.Builder builder) {
		...
		Orientation orientation = switch (cursor.requiredAttribute("preferredOrientation")) {
