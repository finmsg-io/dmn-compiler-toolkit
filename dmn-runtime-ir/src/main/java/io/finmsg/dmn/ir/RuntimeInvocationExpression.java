...
/** Invocation of either a statically named function or a dynamically evaluated target. */
public record RuntimeInvocationExpression(
    Optional<String> function,
    Optional<RuntimeExpression> target,
    List<RuntimeNamedArgument> namedArguments,
    List<RuntimeExpression> positionalArguments,
  ...
  public RuntimeInvocationExpression {
    ...
    if (function.isPresent() == target.isPresent()) {
      throw new IllegalArgumentException(
          "Exactly one of function and target must be present.");
    }
