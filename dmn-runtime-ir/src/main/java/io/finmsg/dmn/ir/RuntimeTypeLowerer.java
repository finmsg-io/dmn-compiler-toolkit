...
final class RuntimeTypeLowerer {
  ...
  private static RuntimeType lower(
    ...
    return switch (type.getKindCase()) {
      ...
      case LIST -> RuntimeType.element(RuntimeTypeKind.LIST,
          lower(type.getList().getElementType(), items, resolving));
      case RANGE -> RuntimeType.element(RuntimeTypeKind.RANGE,
          lower(type.getRange().getElementType(), items, resolving));
  ...
  static RuntimeType lower(FeelType type, Map<String, ItemDefinition> items) {
    return switch (type.getTypeCase()) {
      ...
      case RANGE -> RuntimeType.element(RuntimeTypeKind.RANGE,
          lower(type.getRange().getElementType(), items));
      case LIST -> RuntimeType.element(RuntimeTypeKind.LIST,
          lower(type.getList().getElementType(), items));
      case CONTEXT -> {
  ...
  static int resolvedFieldIndex(RuntimeType source, String member) {
    ...
    return candidate.fieldLayout().stream()
        ...
        .map(RuntimeField::index).findFirst().orElse(-1);
  }

  private static RuntimeType lowerContext(
      ContextTypeReference context, Map<String, ItemDefinition> items, Set<String> resolving) {
    List<RuntimeField> fields = new ArrayList<>();
    for (int index = 0; index < context.getEntriesCount(); index++) {
      ...
      fields.add(new RuntimeField(index, entry.getName(),
          lower(entry.getType(), items, new HashSet<>(resolving))));
    }
