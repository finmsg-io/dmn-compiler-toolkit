...
final class RuntimeIrValidation {
  ...
  static void validateModel(
    ...
    decisions.forEach(value -> addAddress(
        value.id(), value.resultSlot(), ids, slots, valueSlotCount));
    bkms.forEach(value -> addAddress(value.id(), value.resultSlot(), ids, slots, valueSlotCount));
    if (slots.size() != valueSlotCount) {
      throw new IllegalArgumentException(
          "valueSlotCount must equal the number of declared runtime values.");
    }
    ...
    decisions.forEach(value -> {
      ...
      value.expression().ifPresent(expression -> validateExpression(
          expression, List.of(value.localSlotCount()), valueSlotCount));
      value.decisionTable().ifPresent(table -> validateTable(
          table, List.of(value.localSlotCount()), valueSlotCount));
    });
    bkms.forEach(value -> {
      validateDependencies(value.id(), value.dependencies(), ids);
      value.function().ifPresent(function ->
          validateExpression(function, List.of(), valueSlotCount));
    });
  ...
  private static void addAddress(
      int id, int slot, Set<Integer> ids, Set<Integer> slots, int valueSlotCount) {
    if (id < 0 || !ids.add(id)) {
  ...
  private static void validateTable(
      RuntimeDecisionTable table, List<Integer> frames, int valueSlotCount) {
  ...
  private static void validateTests(
      RuntimeUnaryTests tests, List<Integer> frames, int valueSlotCount) {
    tests.tests().forEach(test -> {
      switch (test) {
        case RuntimeComparisonUnaryTest comparison ->
            validateExpression(comparison.endpoint(), frames, valueSlotCount);
        case RuntimeRangeUnaryTest range ->
            validateExpression(range.range(), frames, valueSlotCount);
  ...
  private static void validateExpression(
      RuntimeExpression expression, List<Integer> frames, int valueSlotCount) {
    switch (expression) {
      case RuntimeValueReference reference -> validateValueSlot(reference.sourceSlot(), valueSlotCount);
      case RuntimeDecisionTableReference reference ->
          validateValueSlot(reference.decisionSlot(), valueSlotCount);
      ...
      case RuntimeListExpression list -> list.elements().forEach(
          value -> validateExpression(value, frames, valueSlotCount));
      case RuntimeFunctionCall call -> call.arguments().forEach(
          value -> validateExpression(value, frames, valueSlotCount));
      case RuntimeContextExpression context -> context.entries().forEach(
          entry -> validateExpression(entry.expression(), frames, valueSlotCount));
      ...
      case RuntimeQuantifiedExpression value -> {
        value.bindings().forEach(binding ->
            validateExpression(binding.source(), frames, valueSlotCount));
      ...
      case RuntimeInvocationExpression invocation -> {
        ...
        invocation.namedArguments().forEach(argument ->
            validateExpression(argument.expression(), frames, valueSlotCount));
        invocation.positionalArguments().forEach(value ->
            validateExpression(value, frames, valueSlotCount));
      }
      case RuntimeRelationExpression relation -> relation.rows().forEach(row ->
          row.forEach(value -> validateExpression(value, frames, valueSlotCount)));
