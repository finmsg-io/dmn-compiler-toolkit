...
/** Generator emitting ultra-lean Java gRPC service stub classes backed by compiled Java decision engines. */
public class DmnGrpcGenerator {

  public DmnGrpcGeneratorResult generate(
      DmnCompilationResult compilation, String compiledEngineFqcn, DmnGrpcGeneratorOptions options) {
    ...
    sb.append("  private final ").append(compiledEngineFqcn).append(" engine = new ")
      .append(compiledEngineFqcn).append("();\n\n");

    sb.append("  @Override\n");
    sb.append("  public void evaluate(DmnEvaluationRequest request, StreamObserver<DmnEvaluationResponse> responseObserver) {\n");
    sb.append("    try {\n");
    ...
    for (Map.Entry<String, Integer> entry : inputSlots.entrySet()) {
      ...
      sb.append("        slots[").append(entry.getValue()).append("] = DmnGrpcValueConverter.toJavaObject(reqInputs.get(\"")
    ...
    sb.append("      responseObserver.onError(io.grpc.Status.INTERNAL.withDescription(exception.getMessage()).withCause(exception).asRuntimeException());\n");
    sb.append("    }\n");
    sb.append("  }\n");
    sb.append("}\n");
