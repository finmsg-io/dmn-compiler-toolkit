...
public final class JavaExpressionEmitter {
  ...
  public static String emit(RuntimeExpression expr) {
    ...
    return switch (expr) {
      ...
      case RuntimeBetweenExpression btn -> "(" + emit(btn.value()) + " != null && compare("
          + emit(btn.value()) + ", " + emit(btn.lower()) + ") >= 0 && compare("
          + emit(btn.value()) + ", " + emit(btn.upper()) + ") <= 0)";
  ...
  private static String emitFunctionDefinition(RuntimeFunctionDefinition fnDef) {
    if (fnDef.body().isEmpty()) return "null";
  ...
  private static String emitQuantified(RuntimeQuantifiedExpression quant) {
    ...
    if (quant.bindings().isEmpty()) return String.valueOf(isEvery);
    return emitNestedQuantified(isEvery, quant.bindings(), 0, quant.satisfies());
  }

  private static String emitNestedQuantified(boolean isEvery, List<RuntimeQuantifiedBinding> bindings, int index, RuntimeExpression satisfies) {
  ...
  private static String emitFor(RuntimeForExpression forExpr) {
    if (forExpr.iterations().isEmpty()) return "List.of()";
  ...
  private static String emitRange(RuntimeRangeExpression range) {
    ...
    return "new RuntimeRangeValue(" + low + ", " + up + ", "
        + range.lowerBoundary() + ", " + range.upperBoundary() + ")";
  }
  ...
  private static String emitContext(RuntimeContextExpression ctx) {
    ...
    if (isPureMap) {
      ...
      for (int i = 0; i < ctx.entries().size(); i++) {
        ...
        sb.append("{\"").append(escapeString(entry.name())).append("\", ").append(emit(entry.expression())).append("}");
      }
      sb.append("})");
      return sb.toString();
    }
