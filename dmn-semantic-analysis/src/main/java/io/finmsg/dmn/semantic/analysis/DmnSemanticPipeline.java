...
/**
 * Public entry point that executes all semantic-analysis passes in dependency
 * order.
 */
public final class DmnSemanticPipeline {
	...
	DmnSemanticPipeline(DmnSemanticPass<DmnSemanticAnalysisResult> references,
	...
	public DmnSemanticPipelineResult analyze(Definitions parsedModel) {
		...
		return new DmnSemanticPipelineResult(typeResult.model(), dependencyResult.compilationOrder(), diagnostics,
	 ...
	 */
	public DmnSemanticPipelineResult analyze(Definitions parsedModel, Collection<Definitions> availableModels) {
		Objects.requireNonNull(parsedModel, "parsedModel");
		...
		DmnSemanticAnalysisResult referenceResult = new DmnSemanticAnalyzer().analyze(parsedModel, repository);
		DmnSemanticAnalysisResult typeResult = new DmnTypeAnalyzer().analyze(referenceResult.model(), repository);
		DmnDependencyAnalysisResult dependencyResult = new DmnDependencyAnalyzer().analyze(typeResult.model(),
				repository);
	...
	private static void addDiagnostics(List<DmnSemanticDiagnostic> target, Set<DiagnosticKey> seen,
