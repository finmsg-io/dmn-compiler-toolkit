...
public final class BuiltinFeelFunctionRegistry implements FeelFunctionRegistry {
	...
	private static final Map<String, List<FeelFunctionSignature>> SIGNATURES = Map.ofEntries(
			...
			entry("time",
					List.of(signature("time", TIME, ANY), signature("time", TIME, NUMBER, NUMBER, NUMBER),
							signature("time", TIME, NUMBER, NUMBER, NUMBER, ANY))),
			entry("date and time",
					List.of(signature("date and time", DATE_TIME, ANY),
			...
			entry("sum",
					List.of(signature("sum", NUMBER, LIST_NUMBER),
							new FeelFunctionSignature("sum", List.of(NUMBER), NUMBER, true))),
			entry("min",
					List.of(signature("min", ANY, LIST_ANY),
							new FeelFunctionSignature("min", List.of(ANY), ANY, true))),
			entry("max",
					List.of(signature("max", ANY, LIST_ANY),
							new FeelFunctionSignature("max", List.of(ANY), ANY, true))),
			entry("mean",
					List.of(signature("mean", NUMBER, LIST_NUMBER),
							new FeelFunctionSignature("mean", List.of(NUMBER), NUMBER, true))),
			entry("median",
					List.of(signature("median", NUMBER, LIST_NUMBER),
							new FeelFunctionSignature("median", List.of(NUMBER), NUMBER, true))),
			entry("mode",
					List.of(signature("mode", LIST_NUMBER, LIST_NUMBER),
							new FeelFunctionSignature("mode", List.of(NUMBER), LIST_NUMBER, true))),
			entry("stddev",
					List.of(signature("stddev", NUMBER, LIST_NUMBER),
							new FeelFunctionSignature("stddev", List.of(NUMBER), NUMBER, true))),
			entry("abs", signature("abs", ANY, ANY)), entry("sqrt", signature("sqrt", NUMBER, NUMBER)),
			entry("exp", signature("exp", NUMBER, NUMBER)), entry("log", signature("log", NUMBER, NUMBER)),
			entry("modulo", signature("modulo", NUMBER, NUMBER, NUMBER)),
			entry("even", signature("even", BOOLEAN, NUMBER)), entry("odd", signature("odd", BOOLEAN, NUMBER)),
			entry("product",
					List.of(signature("product", NUMBER, LIST_NUMBER),
			...
			entry("matches",
					List.of(signature("matches", BOOLEAN, ANY, ANY), signature("matches", BOOLEAN, ANY, ANY, ANY))),
			entry("replace",
					List.of(signature("replace", STRING, ANY, ANY, ANY),
			...
			entry("ceiling",
					List.of(signature("ceiling", NUMBER, NUMBER), signature("ceiling", NUMBER, NUMBER, NUMBER))),
			...
			entry("all",
					List.of(signature("all", BOOLEAN, list(BOOLEAN)),
							new FeelFunctionSignature("all", List.of(BOOLEAN), BOOLEAN, true))),
			entry("any",
					List.of(signature("any", BOOLEAN, list(BOOLEAN)),
			...
			entry("string join",
					List.of(signature("string join", STRING, LIST_ANY), signature("string join", STRING, LIST_ANY, ANY),
			...
			entry("PMT2", signature("PMT2", NUMBER, NUMBER, NUMBER, NUMBER)), entry("now", signature("now", DATE_TIME)),
			entry("today", signature("today", DATE)), entry("during", signature("during", BOOLEAN, ANY, ANY)),
