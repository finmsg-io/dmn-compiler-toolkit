...
class DmnSemanticPipelineTest {
	...
	void analyzesTrafficViolationEndToEnd() {
		...
		assertThat(result.compilationOrder()).extracting(DmnSemanticPipelineTest::name).containsExactly("Fine",
	...
	void combinesDependencyDiagnosticsAndSuppressesCyclicOrder() {
		Decision first = Decision.newBuilder().setNode(Node.newBuilder().setId("a").setName("A"))
				.addInformationRequirements(requirement("b")).addInformationRequirements(requirement("b")).build();
		Decision second = Decision.newBuilder().setNode(Node.newBuilder().setId("b").setName("B"))
				.addInformationRequirements(requirement("a")).build();
		Definitions model = Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setDecision(first))
				.addDrgElements(DrgElement.newBuilder().setDecision(second)).build();
	...
	void removesDuplicateDiagnosticsReportedByMultiplePasses() {
		Decision decision = Decision.newBuilder().setNode(Node.newBuilder().setId("decision").setName("Decision"))
				.setLogic(DecisionLogic.newBuilder()
						.setLiteralExpression(Feel.newBuilder().setParsed(feelParser.parseExpressionAst("Missing"))))
				.build();
		Definitions model = Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setDecision(decision))
				.build();

		assertThat(pipeline.analyze(model).diagnostics()).extracting(DmnSemanticDiagnostic::code)
				.containsExactly("UNKNOWN_NAME");
	}

	@Test
	void linksImportedReferencesNamesAndTypesByNamespace() {
		TypeReference applicantType = TypeReference.newBuilder()
				.setNamed(NamedTypeReference.newBuilder().setName("Applicant").setNamespace("urn:base")).build();
		Definitions imported = Definitions.newBuilder().setNamespace("urn:base")
				...
				.addDrgElements(DrgElement.newBuilder()
						.setInputData(InputData.newBuilder()
		...
		Decision decision = Decision.newBuilder().setNode(Node.newBuilder().setId("decision").setName("Decision"))
				...
				.addInformationRequirements(InformationRequirement.newBuilder()
						.setInput(ElementReference.newBuilder().setHref("urn:base#external-input")))
				...
				.build();
		Definitions root = Definitions.newBuilder().setNamespace("urn:root")
		...
		assertThat(result.diagnostics()).isEmpty();
		assertThat(result.bindings()).filteredOn(binding -> binding.targetNamespace().equals("urn:base"))
				.extracting(DmnSymbolBinding::symbolId).contains("applicant-type", "external-input");
		assertThat(result.bindings()).filteredOn(binding -> binding.symbolId().equals("external-input"))
				.allSatisfy(binding -> assertThat(binding.type().getNamed().getNamespace()).isEqualTo("urn:base"));
	}
	...
	void reportsUnavailableImportedNamespace() {
		...
		assertThat(pipeline.analyze(root, List.of()).diagnostics()).extracting(DmnSemanticDiagnostic::code)
				.containsExactly("UNKNOWN_IMPORT");
	}

	@Test
	void ordersDependenciesAcrossImportedModels() {
		Decision baseDecision = Decision.newBuilder().setNode(Node.newBuilder().setId("base-decision").setName("Base"))
				.build();
		Definitions imported = Definitions.newBuilder().setNamespace("urn:base")
				.addDrgElements(DrgElement.newBuilder().setDecision(baseDecision)).build();
		Decision rootDecision = Decision.newBuilder().setNode(Node.newBuilder().setId("root-decision").setName("Root"))
				.addInformationRequirements(InformationRequirement.newBuilder()
		...
		assertThat(result.diagnostics()).isEmpty();
		assertThat(result.compilationOrder()).extracting(DmnSemanticPipelineTest::name).containsExactly("Base", "Root");
	}

	private static InformationRequirement requirement(String id) {
		return InformationRequirement.newBuilder().setDecision(ElementReference.newBuilder().setHref("#" + id)).build();
	}
