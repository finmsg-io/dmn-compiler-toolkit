...
class DmnDependencyAnalyzerTest {
  ...
  void ordersDependenciesBeforeConsumers() {
    ...
    Definitions model = Definitions.newBuilder()
        .addDrgElements(second)
        .addDrgElements(first)
        .addDrgElements(derived)
        .addDrgElements(base)
        .build();
    ...
    assertThat(result.compilationOrder()).extracting(DmnDependencyAnalyzerTest::name)
        .containsExactly("Base", "Derived", "First", "Second");
  }

  @Test
  void reportsDecisionCyclesAndSuppressesCompilationOrder() {
    Definitions model = Definitions.newBuilder()
        .addDrgElements(decision("a", "A", "b", null))
        .addDrgElements(decision("b", "B", "a", null))
        .build();
  ...
  void reportsSelfReferencingBkm() {
    Definitions model = Definitions.newBuilder()
        .addDrgElements(bkm("self", "Self", "self"))
        .build();
  ...
  void reportsDuplicateRequirements() {
    ...
    assertThat(result.compilationOrder()).extracting(DmnDependencyAnalyzerTest::name)
        .containsExactly("Dependency", "Consumer");
  }

  private static DrgElement decision(
      String id, String name, String requiredDecision, String requiredBkm) {
  ...
  private static DrgElement bkm(String id, String name, String... requiredBkms) {
    BusinessKnowledgeModel.Builder bkm = BusinessKnowledgeModel.newBuilder()
        .setNode(Node.newBuilder().setId(id).setName(name));
    List.of(requiredBkms).forEach(required ->
        bkm.addKnowledgeRequirements(knowledgeRequirement(required)));
  ...
  private static InformationRequirement informationRequirement(String id) {
    return InformationRequirement.newBuilder()
        .setDecision(reference(id))
        .build();
  }

  private static KnowledgeRequirement knowledgeRequirement(String id) {
    return KnowledgeRequirement.newBuilder()
        .setRequiredKnowledge(reference(id))
        .build();
  }
