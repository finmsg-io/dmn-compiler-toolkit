...
class DmnSemanticAnalyzerTest {
	...
	void reportsInvalidUnavailableAndUnknownNames() {
		ItemDefinition inputType = ItemDefinition.newBuilder().setNode(Node.newBuilder().setName("tInput"))
				.addComponents(ItemComponent.newBuilder().setNode(Node.newBuilder().setName("Known"))).build();

		InputData input = InputData.newBuilder().setNode(Node.newBuilder().setId("input-id").setName("Input"))
				.setVariable(InformationItem.newBuilder().setType(namedType("tInput"))).build();

		Decision hidden = Decision.newBuilder().setNode(Node.newBuilder().setId("hidden-id").setName("Hidden")).build();

		Decision target = Decision.newBuilder().setNode(Node.newBuilder().setId("target-id").setName("Target"))
				.addInformationRequirements(InformationRequirement.newBuilder()
						.setInput(ElementReference.newBuilder().setHref("#input-id")))
				.setLogic(
						DecisionLogic.newBuilder().setLiteralExpression(parsedFeel("Input.Unknown + Hidden + Missing")))
				.build();

		Definitions model = Definitions.newBuilder().addItemDefinitions(inputType)
				.addDrgElements(DrgElement.newBuilder().setInputData(input))
				.addDrgElements(DrgElement.newBuilder().setDecision(hidden))
				.addDrgElements(DrgElement.newBuilder().setDecision(target)).build();

		DmnSemanticAnalysisResult result = analyzer.analyze(model);

		assertThat(result.model()).isSameAs(model);
		assertThat(result.isSuccess()).isFalse();
		assertThat(result.diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly("INVALID_PROPERTY",
				"UNAVAILABLE_NAME", "UNKNOWN_NAME");
		assertThat(result.diagnostics()).extracting(DmnSemanticDiagnostic::message).containsExactly(
				"Type 'tInput' has no property 'Unknown'.",
				"Name 'Hidden' exists but is not available through a requirement.", "Unknown name 'Missing'.");
	}
	...
	void reportsDuplicateGlobalNames() {
		Definitions model = Definitions.newBuilder()
				.addDrgElements(DrgElement.newBuilder().setInputData(
						InputData.newBuilder().setNode(Node.newBuilder().setId("first").setName("Duplicate"))))
				.addDrgElements(DrgElement.newBuilder().setDecision(
						Decision.newBuilder().setNode(Node.newBuilder().setId("second").setName("Duplicate"))))
				.build();
	...
	void validatesDeclaredTypesAcrossDrgElementsAndDecisionTables() {
		InputData input = InputData.newBuilder().setNode(Node.newBuilder().setName("Input"))
				.setVariable(InformationItem.newBuilder().setType(namedType("MissingInputType"))).build();
		DecisionTable table = DecisionTable.newBuilder()
				.addInputs(InputClause.newBuilder().setType(namedType("MissingInputClauseType")))
				.addOutputs(OutputClause.newBuilder().setType(namedType("MissingOutputClauseType"))).build();
		Decision decision = Decision.newBuilder().setNode(Node.newBuilder().setName("Decision"))
		...
		DmnSemanticAnalysisResult result = analyzer
				.analyze(Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setInputData(input))
		...
		assertThat(result.diagnostics()).extracting(DmnSemanticDiagnostic::path).containsExactly(
				"definitions/inputData[Input]/variable/type", "definitions/decision[Decision]/variable/type",
	...
	void validatesItemDefinitionTypeGraph() {
		ItemDefinition invalid = ItemDefinition.newBuilder().setNode(Node.newBuilder().setName("Invalid"))
				.setType(namedType("Missing"))
				.addComponents(ItemComponent.newBuilder().setNode(Node.newBuilder().setName("value"))
						.setType(namedType("number")))
				.addComponents(ItemComponent.newBuilder().setNode(Node.newBuilder().setName("value"))
						.setType(namedType("string")))
				.build();
		ItemDefinition first = ItemDefinition.newBuilder().setNode(Node.newBuilder().setName("First"))
				.setType(namedType("Second")).build();
		ItemDefinition second = ItemDefinition.newBuilder().setNode(Node.newBuilder().setName("Second"))
				.setType(namedType("First")).build();

		DmnSemanticAnalysisResult result = analyzer.analyze(Definitions.newBuilder().addItemDefinitions(invalid)
				.addItemDefinitions(first).addItemDefinitions(second).build());

		assertThat(result.diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly("UNKNOWN_TYPE",
				"DUPLICATE_COMPONENT", "CYCLIC_TYPE_DEFINITION");
	...
	void exposesResolvedElementAndFeelNameBindings() {
		InputData input = InputData.newBuilder().setNode(Node.newBuilder().setId("input-id").setName("ApplicantAge"))
				.setVariable(InformationItem.newBuilder().setType(builtin(BuiltinType.BUILTIN_TYPE_NUMBER))).build();
		Decision decision = Decision.newBuilder().setNode(Node.newBuilder().setId("decision-id").setName("Eligibility"))
				.addInformationRequirements(InformationRequirement.newBuilder().setInput(reference("#input-id")))
				.setLogic(DecisionLogic.newBuilder().setLiteralExpression(parsedFeel("ApplicantAge"))).build();
		...
		assertThat(result.bindings())
				.extracting(DmnSymbolBinding::referencePath, DmnSymbolBinding::declarationPath,
						DmnSymbolBinding::symbolId, DmnSymbolBinding::kind)
				.containsExactly(
						org.assertj.core.groups.Tuple.tuple(
								"definitions/decision[Eligibility]/informationRequirement[0]",
								"definitions/drgElement[input-id]", "input-id", DmnSymbolKind.INPUT_DATA),
						org.assertj.core.groups.Tuple.tuple("definitions/decision[Eligibility]/logic/literalExpression",
								"definitions/drgElement[input-id]", "input-id", DmnSymbolKind.INPUT_DATA));
	}
	...
	void exposesResolvedNamedTypeBindings() {
		ItemDefinition applicant = ItemDefinition.newBuilder()
				.setNode(Node.newBuilder().setId("applicant-type").setName("Applicant")).build();
		InputData input = InputData.newBuilder().setNode(Node.newBuilder().setId("input-id").setName("Applicant data"))
				.setVariable(InformationItem.newBuilder().setType(namedType("Applicant"))).build();

		DmnSemanticAnalysisResult result = analyzer.analyze(Definitions.newBuilder().addItemDefinitions(applicant)
				.addDrgElements(DrgElement.newBuilder().setInputData(input)).build());

		assertThat(result.diagnostics()).isEmpty();
		assertThat(result.bindings()).singleElement().satisfies(binding -> {
			assertThat(binding.referencePath()).isEqualTo("definitions/inputData[Applicant data]/variable/type");
			assertThat(binding.declarationPath()).isEqualTo("definitions/itemDefinition[Applicant]");
	...
	void reportsDuplicateDrgElementIds() {
		Definitions model = Definitions.newBuilder()
				.addDrgElements(DrgElement.newBuilder().setInputData(
						InputData.newBuilder().setNode(Node.newBuilder().setId("duplicate-id").setName("First"))))
				.addDrgElements(DrgElement.newBuilder().setDecision(
						Decision.newBuilder().setNode(Node.newBuilder().setId("duplicate-id").setName("Second"))))
				.build();
	...
	void validatesRequirementAndDecisionServiceReferences() {
		InputData input = InputData.newBuilder().setNode(Node.newBuilder().setId("input-id").setName("Input")).build();
		Decision dependency = Decision.newBuilder()
				.setNode(Node.newBuilder().setId("decision-id").setName("Dependency")).build();
		Decision target = Decision.newBuilder().setNode(Node.newBuilder().setId("target-id").setName("Target"))
				.addInformationRequirements(InformationRequirement.newBuilder().setInput(reference("#decision-id")))
				.addKnowledgeRequirements(
						KnowledgeRequirement.newBuilder().setRequiredKnowledge(reference("#missing-bkm")))
				.addAuthorityRequirements(
						AuthorityRequirement.newBuilder().setRequiredAuthority(reference("#input-id")))
				.build();
		KnowledgeSource authority = KnowledgeSource.newBuilder()
				.setNode(Node.newBuilder().setId("authority-id").setName("Authority")).build();
		DecisionService service = DecisionService.newBuilder()
				.setNode(Node.newBuilder().setId("service-id").setName("Service"))
				.addOutputDecisions(reference("#input-id")).addInputData(reference("#missing-input")).build();

		Definitions model = Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setInputData(input))
		...
		assertThat(analyzer.analyze(model).diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly(
				"INVALID_REFERENCE_KIND", "UNKNOWN_REFERENCE", "INVALID_REFERENCE_KIND", "INVALID_REFERENCE_KIND",
				"UNKNOWN_REFERENCE");
	}

	@Test
	void requiresDecisionServiceOutputDecision() {
		DecisionService service = DecisionService.newBuilder().setNode(Node.newBuilder().setName("Empty service"))
				.build();

		assertThat(analyzer.analyze(
				Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setDecisionService(service)).build())
				.diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly("MISSING_OUTPUT_DECISION");
	}

	@Test
	void rejectsDuplicateAndConflictingDecisionServiceReferences() {
		Decision decision = Decision.newBuilder().setNode(Node.newBuilder().setId("decision-id").setName("Decision"))
				.build();
		DecisionService service = DecisionService.newBuilder().setNode(Node.newBuilder().setName("Service"))
				.addOutputDecisions(reference("#decision-id")).addOutputDecisions(reference("#decision-id"))
				.addEncapsulatedDecisions(reference("#decision-id")).build();

		DmnSemanticAnalysisResult result = analyzer
				.analyze(Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setDecision(decision))
						.addDrgElements(DrgElement.newBuilder().setDecisionService(service)).build());

		assertThat(result.diagnostics()).extracting(DmnSemanticDiagnostic::code)
				.containsExactly("DUPLICATE_DECISION_SERVICE_REFERENCE", "CONFLICTING_DECISION_SERVICE_ROLE");
	}
	...
	void validatesInvocationBindingsAgainstBkmParameters() {
		BusinessKnowledgeModel bkm = BusinessKnowledgeModel.newBuilder()
				...
				.setFunction(FunctionDefinition.newBuilder().addFormalParameters(parameter("x"))
						.addFormalParameters(parameter("y")).setLogic(parsedFeel("x + y")))
				.build();
		Invocation invocation = Invocation.newBuilder().setExpression(parsedFeel("Calculator"))
				.addBindings(binding("x", "1")).addBindings(binding("x", "2")).addBindings(binding("z", "3")).build();
		Decision target = Decision.newBuilder().setNode(Node.newBuilder().setId("target-id").setName("Target"))
				.addKnowledgeRequirements(KnowledgeRequirement.newBuilder().setRequiredKnowledge(reference("#bkm-id")))
		...
		assertThat(analyzer.analyze(model).diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly(
				"DUPLICATE_INVOCATION_BINDING", "UNKNOWN_INVOCATION_PARAMETER", "MISSING_INVOCATION_BINDING");
	}

	@Test
	void requiresBkmEncapsulatedLogic() {
		BusinessKnowledgeModel bkm = BusinessKnowledgeModel.newBuilder()
				.setNode(Node.newBuilder().setName("Missing logic")).build();

		assertThat(analyzer.analyze(
				Definitions.newBuilder().addDrgElements(DrgElement.newBuilder().setBusinessKnowledgeModel(bkm)).build())
				.diagnostics()).extracting(DmnSemanticDiagnostic::code).containsExactly("MISSING_BKM_FUNCTION");
	}
	...
	void validatesBkmFormalParameterSignature() {
		TypeReference declaredFunction = TypeReference.newBuilder()
				.setFunction(
						FunctionTypeReference.newBuilder().addParameterType(builtin(BuiltinType.BUILTIN_TYPE_NUMBER))
	...
	private Binding binding(String parameter, String expression) {
		return Binding.newBuilder().setParameter(parameter).setExpression(parsedFeel(expression)).build();
	}
}
