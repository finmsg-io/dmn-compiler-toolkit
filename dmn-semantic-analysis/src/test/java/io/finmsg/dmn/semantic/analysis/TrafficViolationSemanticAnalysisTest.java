...
class TrafficViolationSemanticAnalysisTest {
  ...
  void infersTypesAcrossTrafficViolationModel() {
    ...
    assertThat(context.getEntries(0).getExpression().getFeel().getAst()
        .getInferredType().getBuiltin()).isEqualTo(BuiltinType.BUILTIN_TYPE_NUMBER);
    assertThat(context.getEntries(1).getExpression().getFeel().getAst()
        .getInferredType().getBuiltin()).isEqualTo(BuiltinType.BUILTIN_TYPE_STRING);
    assertThat(context.getEntries(1).getExpression().getFeel().getAst()
        .getIfExpression().getCondition().getInferredType().getBuiltin())
        .isEqualTo(BuiltinType.BUILTIN_TYPE_BOOLEAN);
  }
