...
public final class FilesystemDmnModelResolver implements DmnModelResolver {
	...
	public DmnResolutionResult resolve(DmnImportRequest request) {
		...
		return request.resolvedLocation().filter(FilesystemDmnModelResolver::isFileUri)
				.map(id -> resolveFile(request, id)).orElseGet(() -> DmnResolutionResult.missing(request));
	}
	...
	private DmnResolutionResult resolveFile(DmnImportRequest request, DmnSourceId requestedId) {
		...
		try {
			...
			DmnSource source = new DmnSource(new DmnSourceId(realCandidate.toUri()), Files.readAllBytes(realCandidate));
