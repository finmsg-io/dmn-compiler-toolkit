...
/**
 * Immutable, thread-safe facade exposing name-based and Protobuf-native DMN evaluation.
 */
public final class DmnCompiledModel {
  ...
  public DmnCompiledModel(
      String namespace,
      String modelName,
      RuntimeOptimizedModel optimizedModel,
      Map<String, Integer> inputSlots,
      Map<String, Integer> decisionSlots,
      Map<String, RuntimeType> inputTypes,
    ...
    this.metadata = CompiledModelMetadata.newBuilder()
        .setNamespace(namespace == null ? "" : namespace)
        .setModelName(modelName == null ? "" : modelName)
        .putAllInputTypes(inTypeNames)
        .putAllDecisionTypes(decTypeNames)
        .build();
  }
  ...
  public EvaluationResponse evaluateInputs(Map<String, Object> namedInputs) {
    EvaluationRequest.Builder request = EvaluationRequest.newBuilder()
  ...
  public static Object toNativeValue(Value value) {
    ...
    return switch (value.getValueKindCase()) {
      ...
      case TIME_VALUE -> value.getTimeValue().matches(".*[Z+-][0-9:]*$")
          ? OffsetTime.parse(value.getTimeValue()) : LocalTime.parse(value.getTimeValue());
      case DATE_TIME_VALUE -> value.getDateTimeValue().matches(".*[Z+-][0-9:]*$")
          ? OffsetDateTime.parse(value.getDateTimeValue()) : LocalDateTime.parse(value.getDateTimeValue());
      case DURATION_VALUE -> value.getDurationValue().contains("T")
          ? Duration.parse(value.getDurationValue()) : Period.parse(value.getDurationValue());
      case LIST_VALUE -> value.getListValue().getElementsList().stream()
          .map(DmnCompiledModel::toNativeValue).toList();
      case CONTEXT_VALUE -> {
