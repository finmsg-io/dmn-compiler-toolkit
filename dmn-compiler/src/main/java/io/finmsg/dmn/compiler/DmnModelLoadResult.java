...
/** Immutable, deterministically ordered graph of parsed DMN sources and resolved imports. */
public record DmnModelLoadResult(
  ...
  private static final Comparator<DmnImportEdge> EDGE_ORDER = Comparator
      ...
      .thenComparingInt(DmnImportEdge::importIndex)
      .thenComparing(DmnImportEdge::imported);
  ...
  public DmnModelLoadResult {
    ...
    if (importEdges.stream().anyMatch(edge ->
        !loadedIds.contains(edge.importer()) || !loadedIds.contains(edge.imported()))) {
