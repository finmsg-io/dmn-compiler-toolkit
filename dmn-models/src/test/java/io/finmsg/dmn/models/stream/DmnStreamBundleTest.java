...
class DmnStreamBundleTest {
	...
	void streamsFromClasspathMultiFileDmnDirectory() {
		...
		assertThat(bundle.sources().keySet()).containsExactlyInAnyOrder("main-loan.dmn", "credit-score.dmn",
				"applicant-risk.dmn");
	...
	void streamsOriginationsMultiFileDmnFromClasspath() {
		...
		assertThat(bundle.sources()).hasSize(2);
		assertThat(bundle.sources().keySet()).containsExactlyInAnyOrder("Chapter 12 Example.dmn", "Financial.dmn");
	...
	void streamsRankedLoanProductsMultiFileDmnFromClasspath() {
		...
		assertThat(bundle.sources()).hasSize(2);
		assertThat(bundle.sources().keySet()).containsExactlyInAnyOrder("Loan info.dmn",
				"Recommended Loan Products.dmn");
	...
	void streamsFromMapOfInputStreams() {
		...
		Map<String, InputStream> streams = Map.of("root.dmn",
				new ByteArrayInputStream(rootXml.getBytes(StandardCharsets.UTF_8)), "base.dmn",
