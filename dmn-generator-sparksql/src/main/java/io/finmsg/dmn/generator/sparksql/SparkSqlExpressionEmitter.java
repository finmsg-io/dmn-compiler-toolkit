...
public final class SparkSqlExpressionEmitter {
  ...
  public static String emit(RuntimeExpression expr, IntFunction<String> slotNameResolver) {
    ...
    return switch (expr) {
      ...
      case RuntimeConditionalExpression cond -> "CASE WHEN (" + emit(cond.condition(), slotNameResolver)
          + ") THEN (" + emit(cond.thenExpression(), slotNameResolver) + ") ELSE (" + emit(cond.elseExpression(), slotNameResolver) + ") END";
      case RuntimePathExpression path -> "(" + emit(path.source(), slotNameResolver) + "." + sanitizeColumn(path.member()) + ")";
      ...
      case RuntimeBetweenExpression btn -> "(" + emit(btn.value(), slotNameResolver) + " >= "
          + emit(btn.lower(), slotNameResolver) + " AND " + emit(btn.value(), slotNameResolver) + " <= " + emit(btn.upper(), slotNameResolver) + ")";
  ...
  private static String emitContext(RuntimeContextExpression ctx, IntFunction<String> slotNameResolver) {
    if (ctx.entries().isEmpty()) return "named_struct()";
  ...
  private static String emitList(RuntimeListExpression list, IntFunction<String> slotNameResolver) {
    if (list.elements().isEmpty()) return "array()";
  ...
  private static String emitFunctionCall(RuntimeFunctionCall fn, IntFunction<String> slotNameResolver) {
    ...
    return switch (name) {
      ...
      case "substring" -> "substring(" + emitArg(args, 0, slotNameResolver) + ", " + emitArg(args, 1, slotNameResolver) + ")";
      case "contains" -> "contains(" + emitArg(args, 0, slotNameResolver) + ", " + emitArg(args, 1, slotNameResolver) + ")";
      case "starts with" -> "startswith(" + emitArg(args, 0, slotNameResolver) + ", " + emitArg(args, 1, slotNameResolver) + ")";
      case "ends with" -> "endswith(" + emitArg(args, 0, slotNameResolver) + ", " + emitArg(args, 1, slotNameResolver) + ")";
  ...
  public static String emitUnaryTests(String inputCol, RuntimeUnaryTests tests, IntFunction<String> slotNameResolver) {
    ...
    for (int i = 0; i < tests.tests().size(); i++) {
      ...
      if (test instanceof RuntimeExpressionUnaryTest exprTest) {
        sb.append("(").append(inputCol).append(" = ").append(emit(exprTest.expression(), slotNameResolver)).append(")");
        ...
        sb.append("(").append(inputCol).append(" ").append(op).append(" ").append(emit(compTest.endpoint(), slotNameResolver)).append(")");
      } else {
        sb.append("TRUE");
      }
    }
    ...
    for (int r = 0; r < table.rules().size(); r++) {
      ...
      if (rule.outputEntries().isEmpty()) {
        ...
        for (int o = 0; o < table.outputs().size(); o++) {
          ...
          String valExpr = o < rule.outputEntries().size() ? emit(rule.outputEntries().get(o), slotNameResolver) : "NULL";
