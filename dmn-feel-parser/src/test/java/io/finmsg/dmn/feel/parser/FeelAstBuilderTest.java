...
class FeelAstBuilderTest {
	...
	void buildsMultipleIterationAndQuantifiedBindings() {
		var forExpression = parser.parseExpressionAst("for x in [1], y in [2] return x + y").getAst()
				.getForExpression();
		var quantified = parser.parseExpressionAst("some x in [1, 2], y in [3, 4] satisfies x < y").getAst()
				.getQuantified();
	...
	void buildsTypedFunctionDefinition() {
		var function = parser.parseExpressionAst("function(x: number) external x + 1").getAst().getFunctionDefinition();
