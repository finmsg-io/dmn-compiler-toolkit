...
class TrafficViolationIntegrationTest {
  ...
  void parsesAllFeelExpressionsInTrafficViolationModel() {
    ...
    assertThat(semanticFine.getLogic().getDecisionTable()
        .getInputs(0).getInputExpression().hasText()).isTrue();
    assertThat(semanticFine.getLogic().getDecisionTable()
        .getRules(0).getInputEntries(1).hasText()).isTrue();
    ...
    table.getInputsList().forEach(input ->
        assertThat(input.getInputExpression().hasParsed()).isTrue());
    ...
    assertThat(table.getRules(0).getInputEntries(1).getParsed()
        .getTests().getTests(0).hasRange()).isTrue();
    ...
    assertThat(parsedViolation.getComponents(2).getConstraint().getParsed()
        .getTests().getTestsCount()).isEqualTo(3);

    ContextParsed context = decision(parsedModel, "Should the driver be suspended?")
        .getLogic().getBoxedExpression().getParsed().getContext();
    ...
    assertThat(context.getEntries(1).getExpression().getFeel().getAst().hasIfExpression())
        .isTrue();
  ...
  private static Decision decision(Definitions definitions, String name) {
    return definitions.getDrgElementsList().stream()
        ...
        .filter(value -> value.getNode().getName().equals(name))
        .findFirst()
  ...
  private static ItemDefinition itemDefinition(Definitions definitions, String name) {
    return definitions.getItemDefinitionsList().stream()
        .filter(value -> value.getNode().getName().equals(name))
        .findFirst()
        .orElseThrow(() -> new AssertionError("Item definition not found: " + name));
  }
}
