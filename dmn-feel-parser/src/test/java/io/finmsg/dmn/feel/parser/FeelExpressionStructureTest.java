...
class FeelExpressionStructureTest {
	...
	void feelNegationHasHigherPrecedenceThanExponentiation() {
		...
		var exponentiation = additive(root).multiplicativeExpression(0).exponentiationExpression(0);
	...
	void postfixOperationsRemainInSourceOrder() {
		...
		var postfix = additive(root).multiplicativeExpression(0).exponentiationExpression(0).arithmeticNegation(0)
				.instanceOfExpression().postfixExpression();
	...
	private static FeelParser.AdditiveExpressionContext additive(FeelParser.ExpressionRootContext root) {
		return root.expression().textualExpression().disjunction().conjunction(0).comparison(0).additiveExpression();
	}
}
