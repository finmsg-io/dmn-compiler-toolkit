...
public final class FeelParseException extends IllegalArgumentException {
	...
	private static String format(List<FeelDiagnostic> diagnostics) {
		return diagnostics.stream().map(d -> "line %d:%d %s".formatted(d.line(), d.charPositionInLine(), d.message()))
				.reduce((left, right) -> left + System.lineSeparator() + right).orElse("unknown syntax error");
	}
}
