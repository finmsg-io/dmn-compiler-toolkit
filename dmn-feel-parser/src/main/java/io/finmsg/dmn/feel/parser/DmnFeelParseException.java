...
/** Thrown by the strict DMN FEEL parsing API when one or more FEEL nodes are invalid. */
    ...
    String details = diagnostics.stream()
        .map(diagnostic -> diagnostic.path() + ": "
            + diagnostic.diagnostics().stream()
                .map(value -> "line %d:%d %s".formatted(
                    value.line(), value.charPositionInLine(), value.message()))
                .reduce((left, right) -> left + "; " + right)
                .orElse("unknown syntax error"))
        .reduce((left, right) -> left + System.lineSeparator() + right)
        .orElse("unknown FEEL error");
