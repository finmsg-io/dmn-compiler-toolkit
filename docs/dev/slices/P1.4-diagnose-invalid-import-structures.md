...
- P1.4 initially used the import-specific `DmnImportDiagnostic`; P1.5 subsequently replaced it
  with the shared compiler diagnostic contract while preserving its classifications;
...
`DMN-IMPORT-DUPLICATE`, and `DMN-IMPORT-CYCLE` through immutable diagnostic values.
P1.5 later migrated those values to `DmnCompilerDiagnostic` without changing their codes or
graph behavior. `DmnModelLoadResult` retains deterministic models, edges, and diagnostics and
explicitly reports validity. Independent invalid branches
