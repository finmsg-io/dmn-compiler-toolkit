...
class DataQualityCorpusTest {
	...
	void testFieldValidationValidPayload() throws Exception {
		ReferenceModelRegistry.CompiledModelHolder holder = ReferenceModelRegistry
				.loadFromClasspath("models/dq-field-validation.dmn");

		RuntimeModel model = holder.compilationResult().optimizedRuntimeModel().orElseThrow().model();
		Map<Integer, Object> inputs = ReferenceModelRegistry.buildInterpreterSlotMap(holder,
				Map.of("email", "john.doe@example.com", "iban", "DE89370400440532013000", "ssn", "123-45-6789"));
	...
	void testCrossFieldConsistencyInvoiceMatching() throws Exception {
		ReferenceModelRegistry.CompiledModelHolder holder = ReferenceModelRegistry
				.loadFromClasspath("models/dq-cross-field-consistency.dmn");

		RuntimeModel model = holder.compilationResult().optimizedRuntimeModel().orElseThrow().model();
		Map<Integer, Object> inputs = ReferenceModelRegistry.buildInterpreterSlotMap(holder, Map.of("subtotal",
				new BigDecimal("100.00"), "tax", new BigDecimal("20.00"), "total", new BigDecimal("120.00")));
	...
	void testDataQualityScoringPass() throws Exception {
		ReferenceModelRegistry.CompiledModelHolder holder = ReferenceModelRegistry
				.loadFromClasspath("models/dq-scoring.dmn");

		RuntimeModel model = holder.compilationResult().optimizedRuntimeModel().orElseThrow().model();
		Map<Integer, Object> inputs = ReferenceModelRegistry.buildInterpreterSlotMap(holder,
				Map.of("fieldValid", true, "consistencyValid", true));
