...
/**
 * Registry and provider for reference DMN models used in benchmarks.
 * Discovers and compiles models into both Runtime IR and generated Java bytecode.
 */
public final class ReferenceModelRegistry {

  private static final AtomicInteger COUNTER = new AtomicInteger(1);

  public record CompiledModelHolder(
      String modelName,
      DmnCompilationResult compilationResult,
      Object generatedEngineInstance,
      Method evaluateMethod,
      Map<String, Integer> inputSlotMapping
  ) {}
  ...
  public static CompiledModelHolder compile(String modelName, DmnSource source) throws Exception {
    ...
    String className = "BenchEngine_" + modelName.replaceAll("[^a-zA-Z0-9_]", "_") + "_" + COUNTER.getAndIncrement();
  ...
  private static Class<?> compileInMemory(String fqcn, String code) throws Exception {
    ...
    URLClassLoader classLoader = new URLClassLoader(new URL[]{tempDir.toUri().toURL()}, ReferenceModelRegistry.class.getClassLoader());
  ...
  private static IllegalStateException ancientCompilationError(String name, DmnCompilationResult result) {
    return new IllegalStateException("Compilation failed for reference model " + name + ": " + result.diagnostics());
  }
}
